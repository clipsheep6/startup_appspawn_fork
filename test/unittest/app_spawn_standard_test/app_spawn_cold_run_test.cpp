...
static int ExecvTimeoutStub(const char *pathName, char *const argv[])
{
    ...
    APPSPAWN_LOGV("ExecvLocalProcessStub pathName: %{public}s ", pathName);
    usleep(WAIT_CHILD_RESPONSE_TIMEOUT * 1000 * 1000);  // 1000 1000ms
    return 0;
}
...
int ExecvFailStub(const char *pathName, char *const argv[])
{
    if (!(strcmp(pathName, "/system/bin/appspawn") == 0 || strcmp(pathName, "/system/asan/bin/appspawn") == 0)) {
        return 0;
    }
    APPSPAWN_LOGV("ExecvFailStub pathName: %{public}s ", pathName);
    return -1;
}
...
HWTEST(AppSpawnColdRunTest, App_Spawn_Cold_Run_005, TestSize.Level0)
{
    ...
    do {
        ...
        ret = -1;
        node->flags |= STUB_NEED_CHECK;
        node->arg = reinterpret_cast<void *>(ExecvFailStub);
        AppSpawnResult result = {};
        ret = AppSpawnClientSendMsg(clientHandle, reqHandle, &result);
        APPSPAWN_LOGV("App_Spawn_Cold_Run_005 Kill pid %{public}d %{public}d", result.pid, result.result);
        if (ret == 0 && result.pid > 0) {
            kill(result.pid, SIGKILL);
        }
        ret = result.result;
...
HWTEST(AppSpawnColdRunTest, App_Spawn_Cold_Run_006, TestSize.Level0)
{
    ...
    do {
        ...
        node->arg = reinterpret_cast<void *>(HandleExecvStub);
        ret = AppSpawnClientSendMsg(clientHandle, reqHandle, &result);
        APPSPAWN_LOGV("App_Spawn_Cold_Run_006 Kill pid %{public}d %{public}d", result.pid, result.result);
        if (ret == 0 && result.pid > 0) {
            kill(result.pid, SIGKILL);
        }
    } while (0);
