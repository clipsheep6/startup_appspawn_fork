...
int AppSpawnSocket::ReadSocketMessage(int socketFd, void *buf, int len)
{
    ...
    APPSPAWN_CHECK(rLen >= 0, return -EFAULT, "Read message from fd %{public}d error %zd: %{public}d",
        socketFd, rLen, errno);
...
int AppSpawnSocket::WriteSocketMessage(int socketFd, const void *buf, int len)
{
    ...
    for (ssize_t wLen = 0; remain > 0; offset += wLen, remain -= wLen, written += wLen) {
        ...
        APPSPAWN_LOGV("socket fd %{public}d, wLen %zd", socketFd, wLen);
        bool isRet = (wLen <= 0) && (errno != EINTR);
        APPSPAWN_CHECK(!isRet, return -errno, "Failed to write message to fd %{public}d, error %zd: %{public}d",
            socketFd, wLen, errno);
    }
