 ...
 */
 ...
 static const std:string SILK_JSON_LIBRARY_PATH("/vendor/lib64/silk/libsilk.so.0.1");

 typedef struct TagParseJsonContext {
    std::set<std::string> appList;
 } ParseJsonContext;

 static ParseJsonContext g_silkContext = {};
 ...
 {
    int ret = ParseJsonConfig(SILK_JSON_BASE_PATH.c_str(), SILK_JSON_CONFIG_NAME.c_str(), ParseSilkConfig, &g_silkContext);
 ...
 void LoadSilkLibrary(const char *packageName)
 {
    ...
    for (std::string appName : g_silkContext.appList) {
        if (appName.compare(packageName) == 0) {
            ...
            APPSPAWN_LOGI("Enbale Silk AppName %{public}s result:%{public}s",
