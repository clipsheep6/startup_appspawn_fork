    ...
    char *longProcName, uint32_t longProcNameLen)
{
    ...
    APPSPAWN_CHECK(!isRet, return -EINVAL,
        "strncpy_s long name error: %{public}d longProcNameLen %u", errno, longProcNameLen);
...
static int SetUidGid(struct AppSpawnContent_ *content, AppSpawnClient *client)
{
    ...
    APPSPAWN_CHECK(!isRet, return -errno,
        "setgroups failed: %{public}d, gids.size=%u", errno, appProperty->property.gidCount);
    ...
    if (client->cloneFlags & CLONE_NEWPID) {
        ...
        APPSPAWN_CHECK(ret == 0, return -errno, "setgid(%u) failed: %{public}d", appProperty->property.gid, errno);
        ...
        APPSPAWN_CHECK(ret == 0, return -errno, "setuid(%u) failed: %{public}d", appProperty->property.uid, errno);
    } else {
        // set gid
        isRet = setresgid(appProperty->property.gid, appProperty->property.gid, appProperty->property.gid) == -1;
        APPSPAWN_CHECK(!isRet, return -errno, "setgid(%u) failed: %{public}d", appProperty->property.gid, errno);

        /* If the effective user ID is changed from 0 to nonzero,
         * then all capabilities are cleared from the effective set
         */
        isRet = setresuid(appProperty->property.uid, appProperty->property.uid, appProperty->property.uid) == -1;
        APPSPAWN_CHECK(!isRet, return -errno, "setuid(%u) failed: %{public}d", appProperty->property.uid, errno);
    }
#endif
...
static int GetWrapBundleNameValue(struct AppSpawnContent_ *content, AppSpawnClient *client)
{
    ...
    int len = sprintf_s(wrapBundleNameKey, WRAP_VALUE_MAX_LENGTH, "wrap.%{public}s", appProperty->bundleName);
...
static int ColdStartApp(struct AppSpawnContent_ *content, AppSpawnClient *client)
{
    ...
    int len = sprintf_s(buffer, sizeof(buffer), "%{public}d", ((AppSpawnClientExt *)client)->fd[1]);
