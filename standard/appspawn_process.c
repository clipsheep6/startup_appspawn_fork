...
static int EncodeAppClient(AppSpawnClient *client, char *param, int32_t originLen)
{
    ...
    if (appProperty->accountId[0] == '\0') {
        strcpy_s(appProperty->accountId, sizeof(appProperty->accountId), "NULL");
    }
    len = sprintf_s(param + startLen, originLen - startLen, ":%s:%s:%s:%u:%s:%s:%s:%s:%u:%" PRIu64 "",
        ...
        appProperty->accountId, appProperty->hapFlags, appProperty->accessTokenIdEx);
...
int GetAppSpawnClientFromArg(int argc, char *const argv[], AppSpawnClientExt *client)
{
    ...
    ret += GetStringFromArg(NULL, &end, client->property.accountId, sizeof(client->property.accountId));
    ret += GetUInt32FromArg(NULL, &end, &value);
