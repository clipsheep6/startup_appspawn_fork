...
{
    APPSPAWN_LOGI("Info %{public}s id %{public}d code %{public}d ", info, client->client.id, client->property.code);
    APPSPAWN_LOGI("processname %{public}s flags 0x%{public}x",
        client->property.processName, client->property.flags);
    APPSPAWN_LOGI("flags 0x%{public}x cloneFlags 0x%{public}x",
        client->client.flags, client->client.cloneFlags);
    APPSPAWN_LOGI("bundleName %{public}s soPath %{public}s", client->property.bundleName, client->property.soPath);
    // access token
    APPSPAWN_LOGI("Access token info %{public}u accessTokenIdEx %{public}" PRId64 " 0x%{public}x ",
        client->property.accessTokenId,
        client->property.accessTokenIdEx, client->property.hapFlags);
    APPSPAWN_LOGI("Access token apl %{public}s renderCmd %{public}s",
        client->property.apl, client->property.renderCmd);
    APPSPAWN_LOGI("uid %{public}u %{public}u gid count %{public}u",
        client->property.uid, client->property.gid, client->property.gidCount);
