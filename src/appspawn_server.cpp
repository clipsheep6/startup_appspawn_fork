...
static void InstallSigHandler()
{
    ...
    if (err < 0) {
        HiLog::Error(LABEL, "Error installing SIGCHLD handler: %{public}d", strerror_r(errno, err_string, ERR_STRING_SZ));
        return;
    }
    ...
    if (err < 0) {
        HiLog::Error(LABEL, "Error installing SIGHUP handler: %{public}d", strerror_r(errno, err_string, ERR_STRING_SZ));
    }
}
...
void AppSpawnServer::MsgPeer(int connectFd)
{
    ...
    if (msgPeer == nullptr || msgPeer->MsgPeer() != 0) {
        HiLog::Error(LABEL, "Failed to listen connection %d, %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
        return;
    }
    ...
    const uint32_t uid, const uint32_t gid, const uint32_t *gitTable, const uint32_t gidCount)
{
    ...
    if (setgroups(gidCount, reinterpret_cast<const gid_t *>(&gitTable[0])) == -1) {
        HiLog::Error(LABEL, "setgroups failed: %{public}d, gids.size=%{public}u", strerror_r(errno, err_string, ERR_STRING_SZ), gidCount);
        return (-errno);
    }
