...
int ServerSocket::BindSocket(int connectFd)
{
    ...
    if ((setsockopt(connectFd, SOL_SOCKET, SO_REUSEADDR, &reuseAddr, sizeof(reuseAddr)) != 0) ||
        ...
        HiLog::Warn(LABEL, "Server: Failed to set opt of socket %d, err %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
    }

    if (bind(connectFd, reinterpret_cast<struct sockaddr *>(&socketAddr_), socketAddrLen_) < 0) {
        HiLog::Error(LABEL, "Server: Bind socket fd %d, failed: %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
        return -1;
    }

    if (chown(socketAddr_.sun_path, APPSPAWN_ID_ROOT, APPSPAWN_ID_SYSTEM)) {
        HiLog::Error(LABEL, "Server: failed to chown socket fd %d, failed: %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
        return -1;
    }
    if (chmod(socketAddr_.sun_path, SOCKET_PERM)) {
        HiLog::Error(LABEL, "Server: failed to chmod socket fd %d, failed: %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
        if ((unlink(socketAddr_.sun_path) != 0) && (errno != ENOENT)) {
            HiLog::Error(LABEL, "Server: Failed to unlink, err %d", strerror_r(errno, err_string, ERR_STRING_SZ));
        }
        return -1;
...
int ServerSocket::WaitForConnection(int connectFd)
{
    ...
    if ((setsockopt(connFd, SOL_SOCKET, SO_RCVTIMEO, &SOCKET_TIMEOUT, sizeof(SOCKET_TIMEOUT)) < 0) ||
        ...
        HiLog::Warn(LABEL, "Server: Failed to set opt of Connection %d, err %d", connFd, strerror_r(errno, err_string, ERR_STRING_SZ));
    }
