...
int AppSpawnSocket::ReadSocketMessage(int socketFd, void *buf, int len)
{
    ...
    if (rLen < 0) {
        HiLog::Error(LABEL, "Read message from fd %d error %zd: %d", socketFd, rLen, strerror_r(errno, err_string, ERR_STRING_SZ));
        return -1;
    }
...
int AppSpawnSocket::WriteSocketMessage(int socketFd, const void *buf, int len)
{
    ...
    for (ssize_t wLen = 0; remain > 0; offset += wLen, remain -= wLen, written += wLen) {
        ...
        if ((wLen <= 0) && (errno != EINTR)) {
            HiLog::Error(LABEL, "Failed to write message to fd %d, error %zd: %d", socketFd, wLen, strerror_r(errno, err_string, ERR_STRING_SZ));
            return -1;
        }
    }
