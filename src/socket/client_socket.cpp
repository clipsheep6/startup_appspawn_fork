...
int ClientSocket::ConnectSocket(int connectFd)
{
    ...
    if ((setsockopt(connectFd, SOL_SOCKET, SO_RCVTIMEO, &SOCKET_TIMEOUT, sizeof(SOCKET_TIMEOUT)) != 0) ||
        ...
        HiLog::Warn(LABEL, "Client: Failed to set opt of socket %d, err %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
    }

    if (connect(connectFd, reinterpret_cast<struct sockaddr *>(&socketAddr_), socketAddrLen_) < 0) {
        HiLog::Warn(LABEL, "Client: Connect on socket fd %d, failed: %d", connectFd, strerror_r(errno, err_string, ERR_STRING_SZ));
